...
class GazeDeformNetwork(nn.Module):
    def __init__(self, 
                 d_in,
                 d_out,
                 d_hidden,
                 ...
                 gaze_encoder=None,
                 weight_norm=True):
        super(GazeDeformNetwork, self).__init__()

        dims = [d_in] + [d_hidden for _ in range(n_layers)] + [d_out - d_variance]
        
        self.embed_fn_gaze = None
        if gaze_encoder is not None:
            embed_fn, input_ch = get_embedder(gaze_encoder, input_dims=d_in)
            self.embed_fn_gaze = embed_fn
            dims[0] = input_ch
    ...
    def forward(self, inputs, alpha_ratio):
        if self.embed_fn_gaze is not None:
            # Anneal
            inputs = self.embed_fn_gaze(inputs, alpha_ratio)
...
class DeformNetwork(nn.Module):
    def __init__(self,
        ...
        if amb_encoder is not None:
            embed_fn, input_ch = get_embedder(amb_encoder, input_dims=dims_in)
...
class TopoNetwork(nn.Module):
    def __init__(self,
        ...
        if amb_encoder is not None:
            embed_fn, input_ch = get_embedder(amb_encoder, input_dims=d_in)
...
class AppearanceNetwork(nn.Module):
    def __init__(self,
        ...
        if xyz_encoder is not None:
            embedxyz_fn, input_ch = get_embedder(xyz_encoder)
            self.embedxyz_fn = embedxyz_fn
            dims[0] += (input_ch - 3)

        self.embednor_fn = None 
        if nor_encoder is not None:
            embednor_fn, input_ch = get_embedder(nor_encoder)
            self.embednor_fn = embednor_fn
            dims[0] += (input_ch - 3)
    ...
    def forward(self, global_feature, points, normals, view_dirs, feature_vectors, alpha_ratio):
        if self.embedxyz_fn is not None:
            # Anneal
            points = self.embedxyz_fn(points, alpha_ratio)
        if self.embednor_fn is not None:
            # Anneal
            normals = self.embednor_fn(normals, alpha_ratio)
...
class SDFNetwork(nn.Module):
    def __init__(self,
        ...
        if self.use_normal_pred:
            dims = [d_in_1 + d_in_2] + [d_hidden for _ in range(n_layers)] + [d_out + 3]
        else:
        ...
        input_ch_2 = d_in_2
        if xyz_encoder is not None:
            xyz_encoder.eye_bbox = eye_bbox
            embed_fn, input_ch_1 = get_embedder(xyz_encoder, input_dims=d_in_1)
            self.embed_fn_fine = embed_fn
            dims[0] += (input_ch_1 - d_in_1)
        if amb_encoder is not None:
            embed_amb_fn, input_ch_2 = get_embedder(amb_encoder, input_dims=d_in_2)
    ...
    def forward(self, input_pts, topo_coord, alpha_ratio, gaze=None):
        input_pts = input_pts * self.scale
        if self.embed_fn_fine is not None:
            # Anneal
            input_pts = self.embed_fn_fine(input_pts, alpha_ratio, gaze)
    ...
    # Anneal
    def sdf(self, x, topo_coord, alpha_ratio, gaze=None):
        return self.forward(x, topo_coord, alpha_ratio, gaze)[:, :1]


    def sdf_hidden_appearance(self, x, topo_coord, alpha_ratio, gaze=None):
        return self.forward(x, topo_coord, alpha_ratio)


    def gradient(self, x, topo_coord, alpha_ratio, gaze=None):
        x.requires_grad_(True)
        y = self.sdf(x, topo_coord, alpha_ratio, gaze)
