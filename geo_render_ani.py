...
if len(sys.argv) != 4:
    print(
        ...
        "    python geo_render_ani.py <dataset_dir> <exp_dir> <iter_step>\n")
    exit()
...
def run_main(inter_mode):
    dir_path = results_path_ + 'validations_inter_' + inter_mode
    if not os.path.exists(dir_path):
        print('No Dir:', inter_mode)
        return
    ...
    meshes_path = dir_path + '/' + str(iter_num_).zfill(8) + '_'

    side_view_angle = 0
    if side_view_angle != 0:
        save_path = dir_path + '_geo_%d/' % side_view_angle + str(iter_num_).zfill(8) + '_'
        os.makedirs(dir_path + '_geo_%d/' % side_view_angle, exist_ok=True)
        depth_save_path = dir_path + '_depth_%d/' % side_view_angle + str(iter_num_).zfill(8) + '_'
        os.makedirs(dir_path + '_depth_%d/' % side_view_angle, exist_ok=True)
    else:
        save_path = dir_path + '_geo/' + str(iter_num_).zfill(8) + '_'
        os.makedirs(dir_path + '_geo/', exist_ok=True)
        depth_save_path = dir_path + '_depth/' + str(iter_num_).zfill(8) + '_'
        os.makedirs(dir_path + '_depth/', exist_ok=True)
    ...
    i = data_num - 1
    gaze_num = len(os.listdir(dir_path))
    print('Total Gaze:', gaze_num)
    for gaze_idx in range(gaze_num):
        camera_dict = {}
        camera_dict["CameraReso"] = [W_, H_]
        camera_dict["ExterMat"] = np.eye(4)
        camera_dict["InterMat"] = intrinsics_all[i,...]

        render_color_mesh(meshes_path+str(gaze_idx)+'.ply', scale_mats[i,...], extrinsics[i,...], camera_dict, save_path, depth_save_path, str(gaze_idx))
